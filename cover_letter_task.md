...
Your job is to write a tailored cover letter for each job listing in `/jobs/` using details from `cv.md` and the output format in `cover_letter_template.md`.

## What to do
1. Read `cv.md` once: your full pool of experience and projects.
2. Read `cover_letter_template.md` once: JSON fields and rules.
...
4. After finishing all jobs, tell the user to run `python build_cover_letters.py`.
...
## Output Rules
* Output only a single JSON object matching `cover_letter_template.md` (no markdown fences around the JSON, no pre/post commentary).
...
* Do not include letterhead or sign-off in the JSON body. Those are added by `build_cover_letters.py`.

## File Locations
* `cv.md`: Full experience and project pool
* `cover_letter_template.md`: JSON schema and example
* `jobs/*.md`: One file per job listing
* `output/[slug]/cover_letter.json`: Write tailored fields here
* `output/[slug]/cover_letter.docx`: Generated by `build_cover_letters.py` (do not write by hand)